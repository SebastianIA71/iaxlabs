...
export default function LabsCatalog() {
 ...
 <Link
  ...
  className="group flex items-start gap-4 p-4 rounded-2xl border
             ...
             hover:border-[#E6332A]/40 hover:shadow-md transition-all"
>
  {/* FOTO */}
  <div className="w-[72px] h-[72px] shrink-0 rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700">
    {meta?.image
      ? <Image src={meta.image} alt={lab.name} width={72} height={72} className="object-cover w-full h-full" />
      : <div className="w-full h-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center">
        ...
        <div className="flex-[1] border-r border-slate-200 dark:border-slate-700 pr-5 flex flex-col justify-center gap-1.5">
          {meta?.topicTitle && (
            <p className="text-[17px] font-semibold leading-snug">{meta.topicTitle}</p>
          )}
      {meta?.topicDescription && (
        <p className="text-[14px] text-slate-400 leading-snug">{meta.topicDescription}</p>
      )}
        </div>
        ...
        <div className="flex-[1] min-w-0 pl-5 flex flex-col justify-center gap-1">
          <div className="flex justify-between items-start">
            ...
            <span className="text-[10px] text-slate-400 uppercase tracking-wider ml-4 shrink-0">{lab.type}</span>
          </div>
          <p className="text-[14px] font-medium group-hover:text-[#E6332A] transition leading-snug">{lab.name}</p>
          {lab.subtitle && (
            <p className="text-[12px] text-slate-500 leading-tight">{lab.subtitle}</p>
          )}
          {lab.topic && (
